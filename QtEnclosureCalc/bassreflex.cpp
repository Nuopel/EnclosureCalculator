...
void bassReflex::lectureFile()
{
    ...
    for(int ii(0);ii<sizeQ;ii++) //sizeQtc
    {
            ...
            if(monFlux)
            {
                ...
                {
                    grille.push_back(QVector<double>(5));
...
void bassReflex::akabakBassReflexScript()
{
    ...
    QString strFileName="GeneratedScript/bassreflex.aks";
...
//
//// Open the file
string const associedParameters((speakerbass->nameParameters()).toStdString());
int nbrConfig=(m_tabParam.size());
    if (nbrConfig==0)
    {
        cout<< "The table of initial parameter is empty"<< endl;
